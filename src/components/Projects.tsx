...
import { Github, ExternalLink, ArrowRight, Grid } from "lucide-react";
import Link from "next/link";
import { useLanguage } from "@/context/LanguageContext";
import projectsData from "@/data/projects.json"; // CARICAMENTO DAL JSON
...
export function ProjectCard({ project, idx }: { project: any, idx: number }) {
  ...
  return (
    ...
    >
      <Link href={`/projects/${project.slug}`} className="block overflow-hidden aspect-video relative h-40">
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-xl font-bold text-white mb-1 group-hover:text-red-500 transition-colors">{project.title}</h3>
        <p className="text-gray-400 text-[11px] leading-relaxed mb-4 flex-grow line-clamp-2 font-light">{project.desc}</p>
...
export function ProjectsGrid() {
  ...
  return (
    <section id="projects" className="h-screen snap-start flex flex-col justify-center px-10 bg-transparent relative overflow-hidden">
      <div className="max-w-7xl mx-auto w-full relative z-10 pt-20">
        <div className="mb-10 flex justify-between items-end">
          <h2 className="text-5xl font-black text-white mb-2 uppercase tracking-tighter drop-shadow-xl leading-tight">
            {t.projects.title}
          </h2>
          <Link href="/projects" className="px-8 py-3 bg-red-950/20 border border-red-800/50 text-red-500 font-bold rounded-full hover:bg-red-900 hover:text-white transition-all uppercase tracking-widest text-[9px] flex items-center gap-3 group shadow-lg">
        ...
        <div className="flex flex-col items-center">
          <a href="https://github.com/tuo-username" target="_blank" rel="noopener noreferrer" className="px-10 py-4 bg-white/5 border border-white/10 rounded-2xl text-gray-400 hover:text-white hover:bg-red-900 hover:border-red-700 transition-all flex items-center gap-3 uppercase tracking-[0.2em] text-[10px] font-bold shadow-lg group">
